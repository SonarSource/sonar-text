...
namespace SonarLint.Secrets.DotNet.UnitTests
{
    ...
    public class EntropyCheckerTest
    {
        ...
        public void thresholdSplitsFalsePositiveGoodEnough() {
            double falsePositivesAboveThreshold = ProcessFile("..\\sonar-text-plugin\\src\\test\\resources\\EntropyChecker\\false-positives.txt", EntropyChecker.ENTROPY_THRESHOLD);
            double truePositivesAboveThreshold = ProcessFile("..\\sonar-text-plugin\\src\\test\\resources\\EntropyChecker\\true-positives.txt", EntropyChecker.ENTROPY_THRESHOLD);
